...
const _sfc_main = /* @__PURE__ */ common_vendor.defineComponent({
  ...
  setup(__props) {
    ...
    const reOrder = async (id) => {
      console.log("再来一单", id);
      try {
        await api_cart.cleanCartAPI().then(() => console.log("购物车已清空"));
        const res = await api_order.reOrderAPI(id);
        if (res.code === 1) {
          common_vendor.index.showToast({
            title: "部分商品库存不足，无法重新下单",
            icon: "none",
            duration: 2e3
          });
          return;
      ...
      return {
        ...
        b: common_vendor.f(historyOrders.value, (item, index, i0) => {
          return common_vendor.e({
            ...
            f: common_vendor.o(($event) => reOrder(item.id), index),
            g: item.status === 2
          }, item.status === 2 ? {
            h: common_vendor.o(($event) => pushOrder(item.id), index)
          } : {}, {
            i: index,
            j: common_vendor.o(($event) => toOrderDetail(item.id), index)
          });
        }),
...
const MiniProgramPage = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["__scopeId", "data-v-73685b36"], ["__file", "D:/code/git/supermarket-manager/hanye-take-out-uniapp/src/pages/history/history.vue"]]);
