...
// 菜品/套餐列表
const dishList = ref<(ProductItem)[]>([])
// // 套餐列表
...
const dialogDish = ref<ProductToCartItem>()
...
const getDishOrSetmealList = async (index: number) => {
  ...
  let res = await getDishListAPI(categoryList.value[index].id)
  console.log(res)
...
// 获取购物车中某个菜品的数量
const getCopies = (dish: ProductItem ) => {
  console.log('getCopies', dish)
	return cartList.value.find((item) => item.productId === dish.id)?.number || 0
...
// Check if the current quantity in cart plus the new addition exceeds available stock
...
console.log(currentCartQuantity)
  ...
  if (form == '购物车') {
    ...
    console.log('普通页面下的dish，点击能直接添加(而不弹出dialog)的菜品说明无口味', item)
      ...
      const partialCart: Partial<CartDTO> = {productId: item.id}
      await addToCartAPI(partialCart)
...
<style lang="less" scoped>
	.secondary .section .bottom-spacer {
	  height: 200rpx; /* 可以根据需要调整高度 */
	  width: 100%; 
	}
.dialog {
  position: fixed;
