...
public class OrderServiceImpl implements OrderService {
    ...
    public OrderSubmitVO submit(OrderSubmitDTO orderSubmitDTO) {
        ...
        for (Cart c : cartList) {
            ...
            //更新库存
            Integer num = productMapper.getNum(c.getProductId()) - c.getNumber();
            productMapper.updatenum(c.getProductId(), num);

            if(num ==0){
                System.out.println("进循环了");
                productMapper.onOff(c.getProductId());
            }
        }
        // 5、向明细表插入n条数据
...
//        }
    ...
    public void reject(OrderRejectionDTO orderRejectionDTO) {
        ...
        //相对应地库存更新
        List<OrderDetail> detailList = orderDetailMapper.getById(order.getId());
        for (OrderDetail d : detailList) {
            //更新库存
            Integer num = productMapper.getNum(d.getProductId()) + d.getNumber();
            productMapper.updatenum(d.getProductId(), num);
            if(num>0){
                productMapper.onOff(d.getProductId());
            }
        }
    ...
    @Override
    public boolean checkOrderStock(Integer id) {
        // 1、先拿到这个订单id的所有菜品
        List<OrderDetail> detailList = orderDetailMapper.getById(id);

        // 2、将订单详情对象转换为购物车对象，并检查库存
        for (OrderDetail x : detailList) {
            // 检查库存
            Product product = productMapper.getById(x.getProductId());
            Integer currentStock = product.getNumber();
            if (currentStock == null || currentStock < x.getNumber()) {
                return false; // 只要有一个商品库存不足就返回false
            }
        }
        return true; // 所有商品库存都充足
    }
