...
public class EmployeeServiceImpl implements EmployeeService {
     ...
     * 注册/新增员工
     */
    public void register(EmployeeRegDTO employeeRegDTO) {
        // 先对用户的密码进行MD5加密，再存到数据库中
        String password = employeeRegDTO.getPassword();
        password = DigestUtils.md5DigestAsHex(password.getBytes());
        employeeRegDTO.setPassword(password);

        Employee employee = new Employee();
        // 将userLoginDTO的属性拷贝到user中
        BeanUtils.copyProperties(employeeRegDTO, employee);
...
//        employee.setGender(1);
        ...
        employee.setCreateUser(employeeRegDTO.getAccount()); // 100表示员工自己注册，此时还不能拿到BaseContext的currentId，只能用100这个数字表示自己了
        employee.setUpdateUser(employeeRegDTO.getAccount());
    ...
    public void addEmployee(EmployeeDTO employeeDTO) {
        ...
        // 为user其他字段填充默认值
        employee.setStatus(1);
        employee.setCreateUser(employeeDTO.getAccount());
        employee.setUpdateUser(employeeDTO.getAccount());
        employee.setCreateTime(LocalDateTime.now());
        employee.setUpdateTime(LocalDateTime.now());
