...
import { getDishPageListAPI, updateDishStatusAPI, deleteDishesAPI,updateDishAPI } from '@/api/product'
...
const multiSelection = ref<product[]>([])
...
const increment = async (row?: any) => {
  if (!row) {
    console.error("row is undefined");
    return;
  }
  row.number += 1;
  await updateDishAPI(row);
};

const decrement = async (row?: any) => {
  if (!row) {
    console.error("row is undefined");
    return;
  }
...
const change_btn = async (row: any) => {
   try {
     ...
     const response = await updateDishStatusAPI(row.id)
     
     console.log('请求返回的响应:', response)
...
<template>
  <el-card>
    <div class="horizontal">
      ...
      <el-input size="large" class="input" v-model="pageData.category" placeholder="请输入商品类别" />
    ...
    <el-table class="table_box" ref="multiTableRef" :data="productList" stripe @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="number" label="库存" align="center" width="200px">
  <template #default="scope">
    <div 
    ...
    >
      ...
      <!-- 固定宽度的数字展示 -->
      <span 
        style="display: inline-block; text-align: center; width: 50px;"
      >
        {{ scope.row.number }}
      </span>
        <!-- 增加按钮 -->
      ...
      <el-table-column label="操作" width="200px" align="center" >
        <template #default="scope">
          <el-button class = "btn2" @click="to_add_update(scope.row)" type="primary">修改</el-button>
          <el-button class = "btn2" @click="change_btn(scope.row)" plain :type="scope.row.status === '1' ? 'danger' : 'primary'">
            {{ scope.row.status === "1" ? '停售' : '起售' }}</el-button>
          <el-button class = "btn2" @click="deleteBatch(scope.row)" type="danger">删除</el-button>
